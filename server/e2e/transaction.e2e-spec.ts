...
describe('Transaction Controller', () => {
    ...
    it('/GET all transactions ', async () => {
        const getEntities: TransactionDTO[] = (await request(app.getHttpServer()).get('/api/transactions').expect(200))
            .body;
    ...
    it('/POST create transactions', async () => {
        const createdEntity: TransactionDTO = (
            await request(app.getHttpServer()).post('/api/transactions').send(entityMock).expect(201)
        ).body;
    ...
    it('/PUT update transactions', async () => {
        const updatedEntity: TransactionDTO = (
            await request(app.getHttpServer()).put('/api/transactions').send(entityMock).expect(201)
        ).body;
