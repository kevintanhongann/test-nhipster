...
describe('Notification Controller', () => {
    ...
    it('/GET all notifications ', async () => {
        const getEntities: NotificationDTO[] = (
    ...
    it('/POST create notifications', async () => {
        const createdEntity: NotificationDTO = (
            await request(app.getHttpServer()).post('/api/notifications').send(entityMock).expect(201)
        ).body;
    ...
    it('/PUT update notifications', async () => {
        const updatedEntity: NotificationDTO = (
            await request(app.getHttpServer()).put('/api/notifications').send(entityMock).expect(201)
        ).body;
