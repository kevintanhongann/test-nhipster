...
describe('Invoice Controller', () => {
    ...
    it('/GET invoices by id', async () => {
        const getEntity: InvoiceDTO = (
            await request(app.getHttpServer())
                .get('/api/invoices/' + entityMock.id)
                .expect(200)
        ).body;
    ...
    it('/POST create invoices', async () => {
        const createdEntity: InvoiceDTO = (
            await request(app.getHttpServer()).post('/api/invoices').send(entityMock).expect(201)
        ).body;
    ...
    it('/PUT update invoices', async () => {
        const updatedEntity: InvoiceDTO = (
            await request(app.getHttpServer()).put('/api/invoices').send(entityMock).expect(201)
        ).body;
    ...
    it('/PUT update invoices from id', async () => {
        const updatedEntity: InvoiceDTO = (
            await request(app.getHttpServer())
                .put('/api/invoices/' + entityMock.id)
                .send(entityMock)
                .expect(201)
        ).body;
    ...
    it('/DELETE invoices', async () => {
        const deletedEntity: InvoiceDTO = (
            await request(app.getHttpServer())
                .delete('/api/invoices/' + entityMock.id)
                .expect(204)
        ).body;
